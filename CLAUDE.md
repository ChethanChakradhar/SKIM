...
2. **Preprocess** — deskew, crop to receipt boundary, boost contrast. Moves accuracy more
   than swapping models does. *(NEXT)*
3. **Extraction** — vision-language model returns strict JSON.
...
**Next — Step 2: Preprocess.** Deskew, crop to the receipt boundary, boost contrast.

**Blocked on:** real receipt photos in `data/raw/`. Do not build Step 2 against synthetic
images — every operation in it is defined by real-world mess (angles, table backgrounds,
folds, faded thermal ink), and synthetic test images will produce code that passes tests
and fails in reality. Ask Chethan for 2-3 real grocery receipt photos, shot casually the way
he'd actually use the app, including at least one deliberately sloppy one.

When building Step 2, show before/after images at each stage so he can see each operation
doing its job rather than taking it on trust.
